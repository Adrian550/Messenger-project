...
Window::Window(QWidget *parent) : QMainWindow(parent)
{
    ...
    if (!QFile::exists(client_database_path))
    {
        if (db.open())
        {
            ...
            query.exec("insert into Connection (ip_address, port) values ('127.0.0.1', 8084)");
        }
    }
...
void Window::on_login_button_clicked()
{
    ...
    if (connection->get_status())
    {
        ...
        else
        {
            ...
            signal += ' ' + user.get_user_id() + ' ' + username_check.toUtf8() + ' ' + password_check.toUtf8();
...
void Window::on_send_button_clicked()
{
    ...
    if (connection->get_status())
    {
        ...
        if (is_message_correct)
        {
            ...
            signal += ' ' + username + ' ' + destination + ' ' + new_message.toUtf8();
...
void Window::format_output(QString mode, QString contact, QByteArray name, QString message)
{
    ...
    if (mode == "message")
    {
        ...
        for (int i = 0; i < message.length(); i++)
        {
            if (message.at(i) > char(32))
            {
                new_message += QString(message.at(i)).toUtf8();
                line_break_count++;
            }
        ...
        new_message.replace("(tort)", "<img src=:/small_emoticons/Resources/small_emoticons/big_tort.png>");
    }
    ...
    if (name == user.get_username())
    {
        ...
        if (*last_message != "user")
        {
            ...
            QString bubble_letter(user.get_username());
            bubble->setText(bubble_letter[0]);
        }
...
void Window::on_add_line_textChanged(const QString &arg1)
{
    ...
    if (connection->get_status())
    {
        if (ui.add_line->text().length() != 0)
        {
            ...
            signal += ' ' + user.get_username() + ' ' + ui.add_line->text().toUtf8();
            connection->send_message(signal);
        }
...
void Window::load_log(QString contact)
{
    ...
    if (itr == chat_map.end())
    {
        ...
        if (query.value(0).toInt() == 0)
        {
            ...
            signal += ' ' + username.toUtf8() + ' ' + contact.toUtf8();
        ...
        else
        {
            ...
            signal += ' ' + username.toUtf8() + ' ' + contact.toUtf8() + ' ' + query.value(0).toByteArray();
...
void Window::on_contact_online(QByteArray contact)
{
    ...
    if (log_count != 0)
    {
        ...
        signal += ' ' + ui.contacts->item(--log_count)->text().toUtf8() + ' ' + user.get_username();
        connection->send_message(signal);
    }
...
void Window::on_update_log(QString contact)
{
    ...
    else
    {
        ...
        if (log_count != 0)
        {
            ...
            signal += ' ' + ui.contacts->item(--log_count)->text().toUtf8() + ' ' + username;
            connection->send_message(signal);
        }
    }
